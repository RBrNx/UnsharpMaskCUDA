...
__device__ void pixel_averageGPU(unsigned char *out, const unsigned char *in, const int x, const int y, const int blur_radius, const unsigned w, const unsigned h, const unsigned nchannels) {
...
__device__ void gaussPixel_averageGPU(unsigned char *out, const unsigned char *in, const int x, const int y, const unsigned w, const unsigned h, const unsigned nchannels, const float *gausskernel, const int blur_radius){
	float red_total = 0, green_total = 0, blue_total = 0;
	const unsigned nsamples = (blur_radius * 2 - 1) * (blur_radius * 2 - 1);
...
__global__ void blurGPU(unsigned char *out, const unsigned char *in, const int blur_radius, const unsigned w, const unsigned h, const unsigned nchannels) {

	int x = blockIdx.x * blockDim.x + threadIdx.x;
	int y = blockIdx.y * blockDim.y + threadIdx.y;

	if (x < w && y < h){
		pixel_averageGPU(out, in, x, y, blur_radius, w, h, nchannels);
	}
}

__global__ void gaussBlurGPU(unsigned char *out, const unsigned char *in, const unsigned w, const unsigned h, const unsigned nchannels, const float *gauss, const int blur_radius){
	int x = blockIdx.x * blockDim.x + threadIdx.x;
	int y = blockIdx.y * blockDim.y + threadIdx.y;

	if (x < w && y < h){
		gaussPixel_averageGPU(out, in, x, y, w, h, nchannels, gauss, blur_radius);
	}
}